import pygame
from pieces import WhiteKing


BOARD_POS = (0, 0)
n = 8
surface_sz = 720           # Proposed physical surface size.
TILESIZE = surface_sz // n    # TILESIZE is length of a square.
brown = (94, 61, 61)
def create_board_surf():
    ...
    for y in range(8):
        for x in range(8):
            ...
            pygame.draw.rect(board_surf, pygame.Color('grey' if dark else 'white'), rect)
            dark = not dark
        dark = not dark
    return board_surf
...
def draw_pieces(screen, board, font):
    for y in range(8):
        for x in range(8):
            ...
            if piece:
                color, type = piece
                white_pawn = font.render(type[0], True, pygame.Color(color))
                black_pawn = pygame.image.load("sprites/blackPawn.png")
                white_pawn = pygame.image.load("sprites/whitePawn.png")
                black_pawn = font.render(type[0], True, pygame.Color('darkgrey'))
                pos = pygame.Rect(BOARD_POS[0] + x * TILESIZE+1, BOARD_POS[1] + y * TILESIZE + 1,
                        TILESIZE, TILESIZE)
                if y == 6:
                    screen.blit(white_pawn, white_pawn.get_rect(center=pos.center).move(1, 1))
                if y == 1:
                    screen.blit(black_pawn, black_pawn.get_rect(center=pos.center))
...
def draw_drag(screen, board, selected_piece, font):
    if selected_piece:
        ...
        # screen.blit(black_pawn, black_pawn.get_rect(center=pos + (1, 1)))
        # screen.blit(white_pawn, white_pawn.get_rect(center=pos))
